...
class QMTClient:
    ...
    def get_kline(self, stock_code: str, period: str, count: int = 200) -> pd.DataFrame:
        ...
        try:
            ...
            # 只有在初次获取或距离上次下载超过 1 小时 (3600秒) 时才触发下载
            if cache_key not in self._last_download_time or (now_ts - self._last_download_time[cache_key] > 3600):
                now_dt = datetime.now()
                ...
                # print(f"已触发 {stock_code} {period} 周期数据下载覆盖")
